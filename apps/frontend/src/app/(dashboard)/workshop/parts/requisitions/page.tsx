...
export default function PartRequisitionsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['part-requisitions', tab, page],
    queryFn: async () => {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Phiếu yêu cầu cấp phát</h1>
        <Link
          href="/workshop/parts/requisitions/new"
                  ...
                  return (
                    <tr key={req.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition">
                      <td className="px-4 py-3">
                        <Link href={`/workshop/parts/requisitions/${req.id}`} className="font-mono text-primary hover:underline">
                          {req.code}
                        </Link>
                      </td>
