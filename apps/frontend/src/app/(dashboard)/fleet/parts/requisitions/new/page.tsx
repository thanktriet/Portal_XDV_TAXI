...
function NewRequisitionForm() {
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      router.push('/fleet/parts/requisitions')
    },
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-6 max-w-4xl">
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Kho cấp phát (Xưởng) *</label>
              <select
                value={toBranchId}
                ...
                required
              >
                <option value="">Chọn kho xưởng</option>
                    ...
                    <div className="col-span-5">
                      ...
                      {selectedPart && (
                        <p className="text-xs text-slate-400 mt-0.5">
                          Đơn vị: {selectedPart.unit} · Tồn kho xưởng: {formatNumber(selectedPart.stockQty)}
                        </p>
                      )}
                    </div>
